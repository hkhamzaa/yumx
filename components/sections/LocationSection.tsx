...
function ContactForm() {
  ...
  function set(k: keyof typeof form, v: string) { setForm(f => ({ ...f, [k]: v })); setError(null) }
  ...
  if (success) {
    return (
      <div className="flex flex-col items-center justify-center gap-3 py-8">
        ...
        <p className="font-display font-bold text-lg text-brand-text uppercase">Message Sent!</p>
        <p className="font-body text-sm text-brand-muted text-center">We&apos;ll get back to you as soon as possible.</p>
        <button onClick={() => { setSuccess(false); setForm({ name: '', email: '', message: '' }) }}
          className="font-body text-xs text-brand-accent hover:underline mt-1">Send another</button>
      </div>
    )
  }
    ...
    <form onSubmit={submit} className="space-y-4" noValidate>
      ...
      <div>
        ...
        <input type="email" value={form.email} onChange={e => set('email', e.target.value)}
          placeholder="ali@example.com" required autoComplete="email" className={inputClass} />
      </div>
      <div>
      ...
      <button type="submit" disabled={loading}
        className={`w-full font-body font-bold text-base py-4 rounded-squoval transition-all flex items-center justify-center gap-2 ${loading ? 'bg-brand-surface-high text-brand-dim cursor-not-allowed' : 'bg-brand-accent text-brand-bg hover:bg-white active:scale-[0.98]'}`}>
