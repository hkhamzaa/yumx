...
export default function OrderConfirmationPage({ params }: { params: { orderNumber: string } }) {
  ...
  useEffect(() => {
    setMounted(true)
    try {
      const stored = sessionStorage.getItem(`yumx_order_${params.orderNumber}`)
      if (stored) {
        setSummary(JSON.parse(stored) as OrderSummary)
        sessionStorage.removeItem(`yumx_order_${params.orderNumber}`)
      }
  ...
  async function fetchLive() {
    try {
      const res = await fetch(`/api/public/orders/status/${params.orderNumber}`)
      ...
      <main className="min-h-screen bg-brand-bg pt-16 md:pt-20 pb-20 flex items-center justify-center px-4">
        <div className="max-w-md w-full text-center">
          ...
          {mounted && summary ? (
            ...
            <p className="font-body text-brand-muted mt-4 mb-6 leading-relaxed">
              Your order <strong className="text-brand-accent font-display">{params.orderNumber}</strong> has been placed.
