  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (mode === 'signup') {
      try {
        ...
        const json = (await res.json()) as { success: boolean; error?: string | Record<string, string[]> }
        if (!json.success) {
          setError(typeof json.error === 'string' ? json.error : 'Sign up failed. Please try again.')
          setLoading(false)
          return
        }
      } catch {
          ...
          <form onSubmit={handleSubmit} className="space-y-4" noValidate>
            ...
            <div>
              <div className="flex items-center justify-between mb-1.5">
                ...
                {mode === 'login' && (
                  <Link href="/forgot-password" className="font-body text-xs text-brand-dim hover:text-brand-accent transition-colors">
                    Forgot password?
                  </Link>
                )}
              </div>
            ...
            <button
              type="submit"
              disabled={loading}
              className={`w-full font-body font-bold text-base py-4 rounded-full transition-all min-h-[52px] ${
                loading
                  ? 'bg-brand-border text-brand-dim cursor-not-allowed'
