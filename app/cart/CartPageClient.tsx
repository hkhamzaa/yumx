...
export function CartPageClient({ sessionUser }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const payload = {
      ...
      guestAddress: form.orderType === 'DELIVERY' ? form.address.trim() : null,
      notes: form.notes.trim() || null,
      items: items.map((i) => ({
        ...(i.menuItemVariantId ? { menuItemVariantId: i.menuItemVariantId } : {}),
        ...(i.comboDealId ? { comboDealId: i.comboDealId } : {}),
        quantity: i.quantity,
      })),
    }
    ...
    try {
      ...
      const json = (await res.json()) as {
        success: boolean
        data?: { orderNumber: string; total: string | number; subtotal: string | number }
        error?: string | FieldError
      }

      if (!json.success) {
        if (typeof json.error === 'object' && json.error !== null) {
          setFieldErrors(json.error as FieldError)
        } else {
          setApiError(typeof json.error === 'string' ? json.error : 'Order failed. Please try again.')
        }
        return
      }

      // Success — persist confirmation data and redirect
      if (json.data) {
        try {
          sessionStorage.setItem(
            `yumx_order_${json.data.orderNumber}`,
            JSON.stringify({
              id: json.data.id,
          ...
          // sessionStorage not available (private browsing) — silently ignore
        }
        clearCart()
